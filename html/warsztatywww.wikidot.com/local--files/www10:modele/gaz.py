...
class czastka:  # ogolnie nie bedziemy uzywac klas, ale tu sie przyda
	# argumenty podawane przy tworzeniu czastki to promien, pozycja i predkosc
	def __init__(self, promien, pos, vel):
		...
		self.deltav = np.array([0.0, 0.0])  #o ile ma sie zmienic predkosc
		#                         w kolejnym kroku symulacji (tez wektor)
...
Tmax = 0.1            # czas, po jakim konczy sie symulacja
...
czasy = []  # lista, ktora bedzie postaci 0,dt,2dt,...,Tmax, nie tworze jej na
# poczekaniu, bo chce aby miala DOKLADNIE tyle samo elementow co lista
# srednieenergie (potrzebne aby narysowac wykres sredniej energii od czasu Esr(t)
...
while t < Tmax:  # glowna petla symulacji
	...
	if (en % 100 == 0):    # bede rysowal obrazek co setny krok symulacji
		...
		for p in particles:        # rysuje wszystkie czastki
			a = plt.gca()      # get current axes, niestety zeby narysowac kolko
			#                    trzeba takich trikowych polecen
	...
	for p in particles:
		...
		# Kolejne zadanie dla Ciebie: a co gdy czastka wyjdzie poza pudelko?
		# Musi wyjsc z drugiej strony!
		
	czasy.append(t)                         # dodaje czas t do listy czasy
	srednieenergie.append(np.mean(energie)) # licze srednia energie i dodaje ja do listy srednieenergie
plt.clf()  # znowu czyszcze wykres
plt.hist(energie, bins=8, color='blue')  # rysuje histogram energii z ostatniego kroku symulacji (8 slupkow)
plt.xlabel("Energia")                    # podpisujemy co jest na osiach!
plt.savefig('histogram.png')             # i juz mamy histogram
plt.clf()
plt.plot(czasy, srednieenergie)  # czas na wykres Esr(t)
# na osi X beda czasy, na osi Y srednieenergie - dlatego musza miec tyle samo elementow!
...
# Na koniec mozesz zrobic gifa z animacja gazu np. linuxowym poleceniem
# convert img* -delay 20 gaz.gif
